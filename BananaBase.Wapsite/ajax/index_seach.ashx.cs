...
namespace Banana.Wapsite.ajax
{
    ...
    /// index_seach 的摘要说明
    /// </summary>
    public class index_seach : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            seach = context.Request.QueryString["seach"].Trim2();
            int pagesize = 12;
            string result = "";
            if (page > 0)
            {
                result = getSeachList(page, pagesize);
            }
            else
            {
                result = "-1";
        ...
        public string getSeachList(int page, int pagesize)
        {    
          ...
          <span class=""prodPrice clf""><i class=""red fl"">￥{4}</i> <i class=""fl"">￥{5}</i></span>
            ...
            var list = new ProductBll().GetAll("*", page, pagesize, " ProductName like'%"+seach+"%'", "", " id").Entity;
