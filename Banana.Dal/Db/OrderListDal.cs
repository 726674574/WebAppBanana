...
{
    public class OrderListDal : DalBase
    {
        #region Auto
        ...
        /// 通过主键查询
        /// </summary>
        public OrderList GetByPrimaryKey(Int32 primaryKey)
        {
            string sql = "select * from [OrderList] with(nolock) where [id] = @id";
            object param = new { id= primaryKey };
            
            using (IDbConnection conn = OpenConnection())
            {
                var r = conn.Query<OrderList>(sql, param);
        ...
        /// 添加一条记录
        /// </summary>
        public int Add(OrderList entity)
        {
        ...
        /// 获取所有
        /// </summary>
        public IList<OrderList> GetAll(string fields, string where, object param, string orderBy)
        {
            string sql = String.Format("select {0} from [OrderList] with(nolock) ", String.IsNullOrEmpty(fields) ? "*" : fields); 
        ...
        /// 获取所有
        /// </summary>
        public IList<OrderList> GetAll(string fields, int pageIndex, int pageSize, string where, object param, string orderBy, out int recordCount)
        {
            StringBuilder sql = new StringBuilder();
            if (!String.IsNullOrEmpty(where))
                where = " where " + where;

            sql.AppendFormat("select count(*) from [OrderList] with(nolock) {0}", where);
            sql.AppendLine();
            
            sql.AppendFormat(@"select *
                                  from (select {0}, row_number() over(order by {1}) as rownum
                                          from [OrderList] with(nolock)
                                          {2} ) as T
            ...
            using (IDbConnection conn = OpenConnection())
            {
                ...
                IList<OrderList> list = r.Read<OrderList>().ToList();
                return list;
            }
        }
        ...
        {              
            string sql = String.Format("update [OrderList] set {0} where {1}", fields, where);
