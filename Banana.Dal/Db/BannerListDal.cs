...
{
    public class BannerListDal : DalBase
    {
        #region Auto
        ...
        /// 通过主键查询
        /// </summary>
        public BannerList GetByPrimaryKey(Int32 primaryKey)
        {
            string sql = "select * from [BannerList] with(nolock) where [id] = @id";
            object param = new { id= primaryKey };
            
            using (IDbConnection conn = OpenConnection())
            {
                var r = conn.Query<BannerList>(sql, param);
        ...
        /// 获取所有
        /// </summary>
        public IList<BannerList> GetAll(string fields, string where, object param, string orderBy)
        {
            string sql = String.Format("select {0} from [BannerList] with(nolock) ", String.IsNullOrEmpty(fields) ? "*" : fields); 
        ...
        /// 获取所有
        /// </summary>
        public IList<BannerList> GetAll(string fields, int pageIndex, int pageSize, string where, object param, string orderBy, out int recordCount)
        {
            StringBuilder sql = new StringBuilder();
            if (!String.IsNullOrEmpty(where))
                where = " where " + where;

            sql.AppendFormat("select count(*) from [BannerList] with(nolock) {0}", where);
            sql.AppendLine();
            
            sql.AppendFormat(@"select *
                                  from (select {0}, row_number() over(order by {1}) as rownum
                                          from [BannerList] with(nolock)
                                          {2} ) as T
            ...
            using (IDbConnection conn = OpenConnection())
            {
                ...
                IList<BannerList> list = r.Read<BannerList>().ToList();
                return list;
            }
        }
        ...
        {              
            string sql = String.Format("update [BannerList] set {0} where {1}", fields, where);
        ...
        {
            string sql=String.Format("delete from [BannerList] where {0}",where);
             using (IDbConnection conn = OpenConnection())
            {
        ...
        {
            string sql=String.Format("Select CAST(MAX({0}) AS float) from [BannerList] ",filed);
              using (IDbConnection conn = OpenConnection())
            {
                try
                {
